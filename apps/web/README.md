...
## Encore: reservaciones PSI

Esta app está adaptada para Encore, un agente dentro de PSI que hereda los permisos del usuario de prueba configurado. El chat puede consultar disponibilidad y preparar una propuesta; **no recibe ninguna herramienta de escritura**. Solo el botón **Aprobar y reservar en PSI** llama al servidor, que crea la reservación real y, después, intenta crear una tarea de seguimiento en Ambiguous AI.

Configura en el `.env` raíz:

```dotenv
MODEL_PROVIDER=openrouter
OPENROUTER_API_KEY=your-key
MODEL=openai/gpt-5.6-sol
PSI_BASE_URL=http://127.0.0.1:3000
PSI_DEV_API_KEY=the-DEV_API_KEY-from-psi-legacy
PSI_USER_EMAIL=test-user.edu.mx
AMBIGUOUS_API_KEY=your-workspace-key
```

No copies `DEV_API_KEY` desde `psi-legacy/.env` al repositorio ni al navegador. Inicia PSI localmente, ejecuta `npm run dev:web`, consulta disponibilidad, propone una reservación y apruébala. La respuesta muestra el ID, `decision` y `motivo` reales de PSI; recarga para leerla de nuevo desde `GET /api/agente/mis-tramites`. Si Ambiguous está configurado, la misma aprobación crea una tarea de seguimiento posterior y muestra su ID o enlace real.
